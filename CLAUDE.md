...
- 研究室のTailnetは費用のため単一アカウントを共有する。Tailscale loginは人を特定しないため、application認可はServerSentinelが発行する個人単位のcredential（既定はWebAuthn/passkey）に依存し、Tailscale identity/device情報は補助扱い
