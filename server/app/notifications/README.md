...
One capacity budget covers a delivery from submission until the owner drains its
completion, so the completion queue always has room and a finished delivery is
handed back rather than dropped, including after `close()`. Its wake-up flag is
lowered only after the owner has observed that queue empty, so no completion is
