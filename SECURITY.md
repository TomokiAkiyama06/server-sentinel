...
- relying-party checks only hold if the dashboard owns its browser origin, with no other application sharing it, as AUTH-011 requires; the same reservation is being recorded on the Issue #6 side, and a co-hosted application on that origin would put the credential within its reach;
