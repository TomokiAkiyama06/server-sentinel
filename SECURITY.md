...
- a retained proxy identity is bounded: the principal keeps at most the last observed value, owner-visible, cleared on revocation or deletion and kept out of diagnostic exports, with longer history in the audit log;
...
- backup eligibility is fixed at registration and an assertion that reports a different value is refused and reported, like a counter regression; backup state is refreshed from every verified assertion, so a credential that syncs after registration stops being shown as not backed up.
