...
ADR 0004 adds the deployment reality that the research-room Tailscale account is shared, so the Tailscale login identifies an account rather than a person and a per-person ServerSentinel credential is what authorizes a human request. The permission model below is unchanged.
